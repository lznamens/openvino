...
ConvolutionKernel_b_fs_yx_fsv16_1x1::ConvolutionTuningData ConvolutionKernel_b_fs_yx_fsv16_1x1::GetTuningParams(const convolution_params& params) const {
    ...
    size_t feature_blocks_count = input.Feature().v % tuning_data.feature_block_size != 0 ?
                                  input.Feature().v / tuning_data.feature_block_size :
                                  CeilDiv(input.Feature().v, tuning_data.feature_block_size);
    ...
    if (feature_blocks_count && !block_size_one_is_better)
        while (feature_blocks_count % (tuning_data.slm_div_factor * 2) == 0 && (tuning_data.slm_div_factor * 2 <= max_slm_div_factor) &&
