...
KERNEL(convolution_b_fs_yx_fsv16_1x1)(
    ...
    const uint lid = (int)get_sub_group_local_id();
...
#else
                src = input[input_offset + k * input_fs_pitch + lid];
...
#if X_BLOCK_SIZE > 1
        for (int i = 0; i < X_BLOCK_SIZE; i++) {
            if (xy * X_BLOCK_SIZE + i >= OUTPUT_SIZE_X * OUTPUT_SIZE_Y || feature_block * FEATURE_SLICE_SIZE + lid >= OUTPUT_FEATURE_NUM)
                return;
...
#else // X_BLOCK_SIZE > 1
        if (feature_block * FEATURE_SLICE_SIZE + lid >= OUTPUT_FEATURE_NUM)
            return;
